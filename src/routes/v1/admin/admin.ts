...
router.use('/', authentication, authorization([RoleCode.ADMIN, RoleCode.SUPERADMIN]));
...
router.get('/:id', validator(schema.userId, ValidationSource.PARAM), adminController.getAdmin);
router.put(
  '/:id',
...
router.delete(
  '/ban/:id',
  authorization([RoleCode.SUPERADMIN, RoleCode.ADMIN]),
  validator(schema.userId, ValidationSource.PARAM),
  validator(schema.banUser),
