...
export default class FileUploadHandler {
  ...
  constructor() {
    const storage = multer.diskStorage({
      ...
      filename: function (req, file, cb) {
        ...
        const dynamicFolder = req.params.mediaFolderName;
        if (dynamicFolder) {
          cb(null, `${dynamicFolder}-${Date.now()}.${file.originalname.split('.').pop()}`);
        } else {
          cb(null, `${generatedFolder}-${Date.now()}.${file.originalname.split('.').pop()}`);
        }
      },
    });
  ...
  public handleMultipleFileUpload(
    ...
    return (req: Request, res: Response, next: NextFunction) => {
      this.upload.fields(fieldNames.map((fieldName) => ({ name: fieldName })))(
        req,
        res,
        (err: any) => {
          if (err instanceof MulterError) {
